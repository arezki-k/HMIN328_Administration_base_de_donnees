...
numINE varchar(10) constraint Etudiant_pk primary key,
nom varchar(20),
prenom varchar(15),
dateNaissance date,
genre varchar(5));
create table Formation (
codeF varchar(9) constraint Formation_pk primary key,
nomF varchar(20),
niveau varchar(3),
nomResponsable varchar(20));

create table InscritDans (
numINE varchar(10),
codeF varchar(9),
anneeInscription number(4),
constraint InscritDans_pk primary key (numINE, codeF, anneeInscription),
constraint InscritDans_fk1 foreign key (numINE) references Etudiant(numINE),
constraint InscritDans_fk2 foreign key (codeF) references Formation(codeF));

create table Apprecie (
  numEtudiant  varchar(10),
  numAmi  varchar(10),
  constraint Apprecie_pk primary key(numEtudiant,numAmi),
  constraint Apprecie_fk1 foreign key (numEtudiant) references Etudiant(numINE),
  constraint Apprecie_fk2 foreign key (numAmi) references Etudiant(numINE));
